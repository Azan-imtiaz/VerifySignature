...
library ECDSA {
    ...
    function _throwError(RecoverError error) private pure {
        ...
        else if (error == RecoverError.InvalidSignature) revert("ECDSA: invalid signature");
        else if (error == RecoverError.InvalidSignatureLength) revert("ECDSA: invalid signature length");
        else if (error == RecoverError.InvalidSignatureS) revert("ECDSA: invalid signature 's' value");
        else if (error == RecoverError.InvalidSignatureV) revert("ECDSA: invalid signature 'v' value");
    }
    ...
    function tryRecover(bytes32 hash, uint8 v, bytes32 r, bytes32 s) internal pure returns (address, RecoverError) {
        bytes32 halfOrder = 0x7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
        if (uint256(s) > uint256(halfOrder)) return (address(0), RecoverError.InvalidSignatureS);
...
contract MCAPresale is Ownable , EIP712{
...
string private constant SIGNING_DOMAIN = "Voucher-Domain";
    ...
    uint256 private usdtRate = 500000000;
    ...
    receive() external payable {
        revert("Use buyTokens with signature");
    }
...
function recover(Voucher calldata voucher) public view returns (address) {
    bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
        keccak256("Voucher(uint256 amount)"),
        voucher.amount
    )));
    ...
    function buyTokens(Voucher  calldata voucher) public payable {
       ...
       require(msg.sender == recover(voucher), "Wrong signature.");
        ...
        totalBonus = totalBonus.add(purchaseBonus);
        tokensSold = tokensSold.add(tokens);
        claimableTokens = claimableTokens.sub(tokens);
    ...
    function withdraw(uint amount) external onlyOwner {
        require(token.balanceOf(address(this)) > 0, "Not enough tokens in contract");
